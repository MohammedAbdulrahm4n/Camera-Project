...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            print(f"❌ فشل في إضافة المستخدم إلى قاعدة البيانات: {e}")
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            flash('حدث خطأ أثناء الاتصال بقاعدة البيانات.')
            return redirect(url_for('login'))
...
def profile():
    ...
    try:
        cursor = mysql.connection.cursor()
        # التعديل هنا لاستخدام PK (المفتاح الأساسي) بدلاً من Index
        cursor.execute('SELECT * FROM users WHERE UserID = %s', (session['user_id'],))
        user = cursor.fetchone()

        # جلب بيانات السجلات من face_logs باستخدام UserID (المفتاح الأساسي)
    ...
    if request.method == 'POST' and 'profile_picture' in request.files:
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                print(f"❌ فشل في تحديث الصورة: {e}")
                flash('حدث خطأ أثناء تحديث الصورة.')
                return redirect(url_for('profile'))
